...
const RATE_LIMIT_KEY = 'arcane-forge-generation-log';
const MAX_GENERATIONS_PER_MINUTE = 5;

const checkClientRateLimit = (): { allowed: boolean; retryAfterSeconds?: number } => {
...
const App: React.FC = () => {
  ...
  const handleGenerate = async () => {
    ...
    if (!rate.allowed) {
      setError(
        `The forge must cool for a moment. Please wait about ${rate.retryAfterSeconds}s before forging another item.`
      );
      return;
    }
